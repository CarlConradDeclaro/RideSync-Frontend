        ...
        <section ref={sectionRef} className='flex flex-col lg:h-lvh md:justify-center items-center lg:flex-row py-16 gap-10'>
            <div className={`lg:w-1/2 flex justify-center items-center ${isInView ? 'animate-slideInFromLeft' : 'opacity-0'}`}>
                <div className="w-1/1 bg-colorBlue rounded-full flex aspect-square lg:aspect-square justify-center">
                    <img src={DriverImg} alt="Picture of a driver of a car with his passenger" className='w-[500px] lg:w-full'/>
                </div>
            </div>
            <div className={`lg:w-1/2 flex justify-center align-center flex-col ${isInView ? 'animate-slideUp' : 'opacity-0'}`}>
                ...
                <Button name="Learn more about being a RideSync Driver" variant="contained" size="large" onClick={handleNavigate}/>
            </div>
        </section>
    )
}
